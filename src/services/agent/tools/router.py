...
class AgentRouter:
    ...
    async def route(
        ...
        preference: Optional[ToolCategory] = None,
    ) -> RoutingDecision:
        ...
        if preference:
            available_tools = [
                t for t in available_tools if t.metadata.category == preference
            ]
    ...
    def _calculate_confidence(
        ...
        cost_penalty = min(tool.metadata.cost_per_call / 0.01, 1.0) if tool.metadata.cost_per_call > 0 else 0
    ...
    def fallback_strategy(
        ...
        available_tools = [
            ...
            if t.metadata.name not in failed_tools
        ]
        ...
        scored_tools = [
            (t, self._calculate_confidence(query, t, None))
